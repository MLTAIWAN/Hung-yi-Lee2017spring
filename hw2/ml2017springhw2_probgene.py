...
def main():
    ...
    train_Xstd = np.zeros(train_X.shape, dtype=np.float64)
    test_Xstd  = np.zeros(test_X.shape, dtype=np.float64)
    ...
    PGmodel = ProbGenerative(n_features=train_Xstd.shape[1])
    ...
    for train_index, vali_index in kf.split(train_Xstd):
        ...
        if PGmodel.fit(trainKF_X, trainKF_y)!= False:
            vali_error.append(PGmodel._get_error(valiKF_X, valiKF_y))
        
    vali_errarr = np.array(vali_error)
    print("validation error list {}".format(vali_errarr))
    ...
    err_ = PGmodel.fit(train_Xstd,train_y)
    print("Ein using all training data is {}".format(err_))
    test_ypred = PGmodel.predict(test_Xstd)
