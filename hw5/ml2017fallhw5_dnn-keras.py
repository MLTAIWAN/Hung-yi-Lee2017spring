#!/bin/env python3A
#*-*code=utf-8*-*
#using DNN to complete the matrix elements prediction
...
from keras.layers import Input, Dense, Dropout, Flatten, Activation, Reshape, Embedding, Dot, Add, Concatenate
...
def get_nn_model(n_users, n_items, latent_dim=6666):
...
def train(batch_size, num_epoch, pretrain, save_every, train_vec, train_rate, val_ratio=0.1, model_name=None, n_user=100, n_movie=100, latent_dim = 666, is_norm=False, rmean=3.0, rdev=1.0):
    ...
    if pretrain == False:
        model = get_nn_model(n_user,n_movie,latent_dim)
    else:
        model = load_model(model_name)

    #prepare for output model filename
    final_f ='model/dnn_model-final.h5'
    if (is_norm==True):
        final_f ='model/dnn_model_norm-final.h5'
    ...
    for e in range(num_epoch):
        ...
        if (e+1) % save_every == 0:
            model.save('model/dnn_model-%d.h5' %(e+1))
            print ('Saved model %s!' %str(e+1))

        if early_stop_counter >= PATIENCE:
            print ('Stop by early stopping')
            print ('Lease loss: '+str(best_metrics))
    ...
    #plt.show()
    if (is_norm==True):
        figfn = "result/dnnloss_rmse_norm_d%d.png" %(latent_dim)
        plt.savefig(figfn)
    else:
        figfn = "result/dnnloss_rmse_d%d.png" %(latent_dim)
        plt.savefig(figfn)
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='ml2017fallhw5_dnn-keras.py')
    ...
    parser.add_argument('--isnorm',dest='is_norm',action='store_true') #if add this option, True. otherwise is False
    ...
    #start to train
    dnn_model = train(batch_size=batch_size, num_epoch=epochs, pretrain=False, save_every=20,
    ...
    user_emb = np.array(dnn_model.layers[2].get_weights()).squeeze()
    print('user embedding shape:{}'.format(user_emb.shape))
    movie_emb = np.array(dnn_model.layers[3].get_weights()).squeeze()
    print('movie embedding shape:{}'.format(movie_emb.shape))
    np.save('user_dnn_emb.npy', user_emb)
    np.save('movie_dnn_emb.npy', movie_emb)

    
    #prediction of training data and calculate loss value
    predict_y = dnn_model.predict([train_X[:,0], train_X[:,1]], verbose=1)
    ...
    print("\nRMSE of training data set is {} for DNN mathod.".format(loss_tot))
    if (args.savetrain==True):
        if (args.is_norm==True):
            predictf = "result/predict_dnn_norm_d%d.csv" %(args.latent_dim)
        elif (args.is_norm==False):
            predictf = "result/predict_dnn_d%d.csv" %(args.latent_dim)
